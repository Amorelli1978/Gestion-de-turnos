...
class FacturasTab(QWidget):
    ...
    def generate_pdf(self):
        ...
        filename = f"Factura_{paciente.replace(' ', '_')}_{fecha}.pdf"
        ...
        c.drawString(50, height - 80, f"Número: {numero}")
        ...
        y -= 20
        for s in servicios:
            c.drawString(50, y, f"{s['codigo']} - {s['descripcion']}")
            c.drawString(250, y, str(s['cantidad']))
            c.drawString(350, y, f"${s['precio_unitario']:.2f}")
    ...
    def init_ui(self):
        ...
        # Tabla de servicios de la factura
        self.servicios_table = QTableWidget()
        self.servicios_table.setColumnCount(5)
        self.servicios_table.setHorizontalHeaderLabels([
            "Servicio", "Cantidad", "Precio Unit.", "Subtotal", "Acción"
        ...
        self.delete_button.clicked.connect(self.delete_factura)
        self.clear_button.clicked.connect(self.clear_form)
        self.print_button.clicked.connect(self.print_factura)
    ...
    def load_combos(self):
        ...
        for servicio in servicios:
            self.servicio_combo.addItem(f"{servicio[1]} - {servicio[2]} (${servicio[3]:.2f})", servicio[0])
    ...
    def update_servicios_table(self):
        ...
        for row, servicio in enumerate(self.servicios_factura):
            self.servicios_table.setItem(row, 0, QTableWidgetItem(f"{servicio['codigo']} - {servicio['descripcion']}"))
            self.servicios_table.setItem(row, 1, QTableWidgetItem(str(servicio['cantidad'])))
            self.servicios_table.setItem(row, 2, QTableWidgetItem(f"${servicio['precio_unitario']:.2f}"))
            self.servicios_table.setItem(row, 3, QTableWidgetItem(f"${servicio['subtotal']:.2f}"))
            
            # Botón eliminar
            remove_button = QPushButton("Eliminar")
            remove_button.clicked.connect(lambda checked, r=row: self.remove_servicio(r))
            self.servicios_table.setCellWidget(row, 4, remove_button)
    ...
    def calculate_total(self):
        total = sum(servicio['subtotal'] for servicio in self.servicios_factura)
        self.total_label.setText(f"Total: ${total:.2f}")
        return total
    ...
    def save_factura(self):
        ...
        try:
            ...
            # Insertar detalles
            for servicio in self.servicios_factura:
                cursor.execute('''
                    INSERT INTO detalle_facturas (factura_id, servicio_id, cantidad, precio_unitario, subtotal)
                    VALUES (?, ?, ?, ?, ?)
                ''', (
    ...
    def print_factura(self):
        if not self.numero_edit.text():
            QMessageBox.warning(self, "Error", "Seleccione una factura para imprimir")
            return
        
        # Aquí se implementaría la funcionalidad de impresión
        QMessageBox.information(self, "Imprimir", "Funcionalidad de impresión - Por implementar")
